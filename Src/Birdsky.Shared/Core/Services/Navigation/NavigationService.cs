...
public class NavigationService : INavigationService
{
	...
	public void Navigate<TViewModel>(object? parameter)
	{
		if (!TryFindViewForViewModel(typeof(TViewModel), out var viewType))
		{
			throw new InvalidOperationException(
				$"ViewModel type {typeof(TViewModel).Name} is not registered for navigation.");
		}
	...
	private bool TryFindViewForViewModel(Type viewModelType, out Type? viewType)
	{
		...
		{
			throw new InvalidOperationException(
				"ViewModel name must end with 'ViewModel' by convention.");
		}

		var viewModelName = viewModelType.Name;
		return _views.TryGetValue(
			viewModelName.Substring(0, viewModelName.Length - "Model".Length), out viewType);
	}
	...
	public void RegisterViewsFromAssembly(Assembly sourceAssembly)
	{
		...
		var pages = sourceAssembly.GetTypes().Where(t => pageType.IsAssignableFrom(t) 
		&& t.Name.EndsWith("View", StringComparison.OrdinalIgnoreCase)).ToArray();
    ...
    public void Initialize()
    {
        SystemNavigationManager.GetForCurrentView().BackRequested 
			+= NavigationManagerBackRequested;
    }
